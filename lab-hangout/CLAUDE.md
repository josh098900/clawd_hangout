...
src/
  ...
  ui/winter.ts         the advent calendar, THE TREE panel (hang an ornament, your presents), wrapping a Secret Santa present, opening one
...
| winter | room state `snowman` `{ day, rolls, deco }` (the Park's snowman, everyone's rolls add up) and `snowfight` `{ t0, by }` (90 s, hits tallied from the snowballs everyone sees); broadcast `snowball` `{ id, x0, y0, x1, y1, hit }` (the thrower decides the hit); the sleigh, tree lighting and New Year from the clock; RPCs in 0018 | see world/winter.ts |
