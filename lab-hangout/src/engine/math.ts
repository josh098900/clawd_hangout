...
export const eOut2 = (u: number): number => 1 - (1 - u) * (1 - u);
...
/** Damped wobble used for squash-and-settle after an impact. */
export const decay = (dt: number, k = 6, w = 18): number => (dt < 0 ? 0 : Math.exp(-dt * k) * Math.cos(dt * w));
