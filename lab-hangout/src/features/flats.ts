...
export async function goHome(): Promise<void> {
  try {
    const f = await game.net.api.flats.mine();
    ...
    if (fresh) game.net.api.flats.save(f.layout).catch((e) => console.warn('[flat]', e));
...
export async function visitFlat(owner: string): Promise<void> {
  ...
  try {
    const f = await game.net.api.flats.get(owner);
...
export async function recheckFlat(): Promise<void> {
  try { const f = await game.net.api.flats.get(FLAT.owner); const same = JSON.stringify(f.layout) === JSON.stringify(FLAT.layout); Object.assign(FLAT, { door: f.door, party: f.party }); if (!same) { FLAT.layout = f.layout; applyLayout(flatRooms()); } }
...
export function onFlatMsg(from: string, f: FlatMsg): void {
  ...
  if (f.k === 'knock' && f.to === game.net.selfId) {
    SFX.dingdong();
    knockPrompt(f.nm, () => { game.net.api.flats.letIn(from).then(() => game.net.send('flat', { k: 'in', to: from, nm: game.me.name })).catch((e) => toast(errText(e))); }, () => game.net.send('flat', { k: 'no', to: from, nm: game.me.name }));
...
async function updateShow(): Promise<void> {
  ...
  try { await game.net.api.flats.save(FLAT.layout); } catch (e) { console.warn('[flat]', e); }
}
function startDecorating(): void {
  ...
  openDecorate({
    ...
    buy: async (id) => { try { const r = await game.net.api.flats.buy(id); FLAT.owned[id] = r.n; game.setTokens(r.tokens); SFX.chime(); quests.stat('furniture'); return true; } catch (e) { toast(errText(e)); SFX.blip(); return false; } },
    save: async () => {
      try { await game.net.api.flats.save(FLAT.layout); game.setState({ k: 'flat', v: { n: Date.now(), party: FLAT.party } }); toast('Saved! Looking good', 2500); SFX.score(); quests.bump('home'); return true; }
      catch (e) { toast(errText(e)); return false; }
    },
    door: () => FLAT.door,
    setDoor: async (d) => { try { await game.net.api.flats.setDoor(d); FLAT.door = d; toast(d === 'locked' ? 'Door locked: people knock and you let them in' : d === 'friends' ? 'Friends can walk right in' : 'Open house: anyone on this server can visit', 3500); } catch (e) { toast(errText(e)); } },
...
export function flatStep(): void {
  ...
  if (game.room.id === 'lofts' && game.playing && Date.now() - dirAt > 8000) {
    ...
    game.net.api.flats.doors(ppl.map((p) => p.id)).then((ds) => { LOFTS_INFO.dir = ds.map((d) => ({ name: ppl.find((p) => p.id === d.owner)?.name ?? d.name, door: d.door, party: !!d.party && d.party > Date.now() / 1000, home: isFlat(ppl.find((p) => p.id === d.owner)?.room ?? 'lab') })).sort((a, b) => Number(b.party) - Number(a.party) || Number(b.home) - Number(a.home)); }).catch(() => {});
  }
