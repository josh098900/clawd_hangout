...
export function refreshGarden(bump = false): void {
  ...
  game.net.plots().then((ps) => { GARDEN.plots = ps; }).catch((e) => console.warn('[garden]', e));
...
function waterBed(n: number): void {
  ...
  gardenDo(() => game.net.water(n), (r) => { if (other) quests.bump('water'); if (r.thanked) quests.stat('helped'); game.setTokens(r.tokens); GARDEN.splash.set(n, now()); SFX.pour(); toast(r.thanked ? 'Watered! +1 token for helping out' : 'Watered! It grows faster for 3 hours', 3000); if (r.thanked) game.floatText('+1'); });
}
export function tendBed(n: number): void {
  ...
  if (!p || plantState(p).dead) {
    ...
    SFX.blip();
    openSeeds(n, game.tokens, (i) => save.has('seed:' + i), (seed) => gardenDo(() => game.net.plant(n, seed), (bal) => {
  ...
  if (p.owner === game.net.selfId) {
    openMyPlant(p, {
      water: () => waterBed(n),
      harvest: () => gardenDo(() => game.net.harvest(n), (r) => {
      ...
      digUp: () => gardenDo(() => game.net.digUp(n), () => toast('Dug up. The bed is free again', 2500)),
    }, () => game.input.clear());
    return;
  }
