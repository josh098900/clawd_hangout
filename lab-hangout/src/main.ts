...
import { MusicPlayer, Rain, FILM_TRACK, PARTY_TRACK, ORBIT_TRACK, TRACKS as LAB_TRACKS, WINTER_TRACKS } from './audio/music';
...
function syncPad(): void {
  ...
  if (i >= 10) { // at a reactor station: turn it down or up
    ...
    const t = document.createElement('span'); t.className = 'pill quiet'; t.textContent = ['SCRAM', 'RODS', 'COOLANT', 'TURBINE'][st];
    pad.replaceChildren(t, mk2('1', '▼ DOWN', -1), mk2('2', '▲ UP', 1));
    return;
  }
...
// what the features (src/features/*) see of the game (getters: always the current room, player, network)
setGame({
