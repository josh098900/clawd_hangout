 ...
 */
export type SpotKind = 'sit' | 'coffee' | 'arcade' | 'juke' | 'board' | 'popcorn' | 'soda' | 'booth' | 'desk' | 'kanban' | 'rack' | 'deploy' | 'party' | 'hammock' | 'scope' | 'fireworks' | 'chest' | 'instrument' | 'fish' | 'marsh' | 'claw' | 'pong' | 'prizes' | 'decor' | 'treat' | 'candle' | 'bed' | 'boat' | 'kite' | 'hotdog' | 'sand' | 'cook' | 'shift' | 'kart' | 'tank' | 'lift' | 'look' | 'flatparty' | 'tray' | 'mission' | 'karaoke' | 'photos';
...
export interface Room {
  ...
  glowMul?(): number;
  /** Weightless right now (the Space Station, the rocket once its engines stop): you drift, bounce off walls, everyone bobs. */
  zeroG?(): boolean;
  ...
  gForce?(): number;
  /** Everyone here is clipped on to a safety line from this point (the spacewalk). */
  tether?: { x: number; y: number };
  /** Things that come and go and depth-sort with players (the spacewalk's stardust and junk). */
